...
import org.pq.esql.config.EsqlConfigManager;
import org.pq.esql.ex.EsqlExecuteException;
import org.pq.esql.ex.EsqlIdNotFoundException;
import org.pq.esql.impl.DbTypeFactory;
import org.pq.esql.impl.EsqlBatch;
import org.pq.esql.impl.EsqlExecInfo;
import org.pq.esql.impl.EsqlRsRetriever;
import org.pq.esql.map.EsqlRowMapper;
...
import org.pq.esql.parser.EsqlDynamicReplacer;
import org.pq.esql.parser.EsqlSqlParser;
...
public class Esql implements Closeable {
    ...
    private void checkPreconditions(String... directSqls) {
        if (esqlItem != null || directSqls.length > 0)
            return;
    ...
    private Object execSub(Object ret, EsqlSub subSql) throws SQLException {
        ...
        try {
            ...
            if (!subSql.getEsqlItem().isOnerrResume())
                throw ex;
        }
    ...
    private Object pageExecute(Object ret, EsqlSub subSql) throws SQLException {
        ...
        if (page.getTotalRows() == 0)
            page.setTotalRows(executeTotalRowsSql(ret, subSql));
    ...
    private void createTotalSql(EsqlSub subSql) {
        ...
        subSql.setSql(fromPos2 > 0 ? "SELECT COUNT(*) CNT__ FROM (" + sql + ")" : "SELECT COUNT(*) AS CNT "
                + sql.substring(fromPos1));
    ...
    private Object execAndRetrieveProcedureRet(EsqlSub subSql, CallableStatement cs) throws SQLException {
        ...
        if (subSql.getOutCount() == 1)
            for (int i = 0, ii = subSql.getPlaceHolders().length; i < ii; ++i)
                if (subSql.getPlaceHolders()[i].getInOut() != InOut.IN)
    ...
    public PreparedStatement prepareSql(EsqlSub subSql, String realSql) throws SQLException {
        ...
        return EsqlUtils.isProcedure(subSql.getSqlType()) ? connection.prepareCall(realSql) : connection
                .prepareStatement(realSql);
    }
    ...
    public Esql(Configable configable) {
    ...
    private void throwError(String sqlId) {
        ...
        if (useSqlFile)
            errMsg.append("in sql file ").append(sqlClassPath);

        throw new EsqlIdNotFoundException(errMsg.toString());
    }

    protected List<EsqlSub> createSqlSubs(String... directSqls) {
        return directSqls.length == 0 ? esqlItem.createSqlSubs(compositeParams(params))
    ...
    private void tranClose() {
        if (internalTran != null)
            RClose.closeQuietly(internalTran);
    ...
    public EsqlTran newTran() {
        EsqlTran tran = newTran(connectionNameOrConfigable);
     ...
     * 创建一个事务对象。
     *
     * @param connectionNameOrConfigable
     * @return
     */
    public static EsqlTran newTran(Object connectionNameOrConfigable) {
        return EsqlConfigManager.getConfig(connectionNameOrConfigable).getTran();
    }
