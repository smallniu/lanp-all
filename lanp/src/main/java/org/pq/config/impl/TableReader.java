...
import org.pq.core.lang.CSVLineReader;
import org.pq.config.bean.ConfigCell;
import org.pq.config.bean.ConfigRow;
import org.pq.config.bean.ConfigTable;
...
public class TableReader {
    ...
    private void doWhenIsData(String line) {
        ...
        if (StringUtils.isEmpty(rowKey)) {
            throw new ConfigException("table [" + tableName + "] config has no rowKey!");
        }
    ...
    private void doWhenIsRowCols(String line) {
        String[] splitLine = StringUtils.split(StringUtils.substring(line, 2, line.length()), ',');
