...
import org.pq.pagestatic.util.StreamGobbler;
import org.slf4j.Logger;

/**
 * 单条rsync命令类。
 * rsync -az /localPathDir/filename  mall@192.168.1.110:/remotePathDir/filename
 *
 */
public class PageRsyncCmd {
    ...
    private String createRemotePath() {
        return new StringBuilder().append(conf.getDestUser()).append('@').append(conf.getDestHost()).append(':')
                .append(dir.getRemoteDir()).toString();
    }
    ...
    public boolean execute() {
        ...
        if (!new File(dir.getLocalDir()).exists())
            return false;
    ...
    private void startStreamGobbler() {
        stdoutStreamGobbler = new StreamGobbler(log, commandLine, process.getInputStream(), StreamGobbler.TYPE.STDOUT);
        stderrStreamGobbler = new StreamGobbler(log, commandLine, process.getErrorStream(), StreamGobbler.TYPE.STDERR);
    ...
    public boolean destroyWhenExpired(long millisOfCheckRsyncExited, long rysncTimeoutMilis) {
        ...
        log.info("{}: {} exited with value {}, cost {} seconds", new Object[] { getLoginUser(), commandLine, exitValue,
